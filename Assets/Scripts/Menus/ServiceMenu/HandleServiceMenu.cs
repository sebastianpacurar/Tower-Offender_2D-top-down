...
namespace Menus.ServiceMenu {
    public class HandleServiceMenu : MonoBehaviour {
        ...
        // if the menu is displayed then force cursor to be displayed, else hide it if the selected weapon IS NOT LightShell
        private void HandleCursorVisibility() {
            if (!menuFadeIn && !menuFadeOut) {
                Cursor.visible = true;
            }
        }

        // NOTE: ShootController is used to handle cursor visibility in InGameMenu.cs
        private void SetShootController() {
            _shootController.enabled = !canvasGroup.interactable;
        }
